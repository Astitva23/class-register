...
1. **Firestore Rules tab** → replace with the current `firestore.rules`
   (drops the allowlist check that was in a previous version — everyone
   with an account can sign in again, no per-username approval step) →
   Publish.
2. If you'd previously created an `allowlist` collection in Firestore's Data
   tab, you can leave it there (it's simply unused now) or delete it — your
   call, it has no effect either way with the current rules.
